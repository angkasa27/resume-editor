...
**6. Actions that read-then-replace must flush first.**
...
`flushOpenForms()` lands it first. Undo/redo/import deliberately do *not* flush —
there, flushing is precisely the clobber invariant 5 prevents.
...
- **The store is the only source of truth for the preview.** A change that
  ...
  nothing.
- **`onSave` throwing is silent.** `parseResumeDraft` runs inside `commit()`, and
  a throw there propagates out of a `setTimeout` callback with no UI. That's why
  the snapshot is updated only *after* `onSave` returns — a failed save stays
  dirty and retries instead of being marked persisted.
...
4. Add an item, type in it, delete it → it stays gone after a reload.
   (Invariant 3.)
5. Type, then hit Sort within 500ms → the typing survives. (Invariant 6.)
