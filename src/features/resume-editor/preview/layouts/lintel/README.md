...
- **Headings** — centred, `h2`, bold, on a band of `color-mix(in srgb, var(--resume-accent) 8%, transparent)` running the full measure. It stops at the page margin, not the paper edge: a band bleeding out would read as a rail, and every section carries one.
...
- **Contacts** — a two-column grid, each field led by an **outlined square glyph** (18px, 1.5px border in the text colour), not a filled chip.
...
- **The tint is `color-mix` on the accent, not a second variable**, so it tracks
  ...
  read as heading text against its own 8% tint. That is what the presets are
  curated for.
